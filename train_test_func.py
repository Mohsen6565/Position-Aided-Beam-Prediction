...
class MY_LSTM(nn.Module):
    def __init__(self, num_classes, input_size, hidden_size, num_layers, seq_length, dropout, my_device):
        ...
        self.fc_1 =  nn.Linear(hidden_size, hidden_size) #fully connected 1ayers
        # self.fc_2 =  nn.Linear(hidden_size, hidden_size) #fully connected 1ayers
    ...
    def forward(self,x):
        h_0 = (t.zeros(self.num_layers, x.size(0), self.hidden_size)).to(self.device) #hidden state
        c_0 = (t.zeros(self.num_layers, x.size(0), self.hidden_size)).to(self.device) #internal state
