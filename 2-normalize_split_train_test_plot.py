...
gathered_data_folder = os.path.join(os.getcwd(), 'Gathered_data3')
...
n_avgs = n_reps                            # number of runs to average                               XXXXX
...
n_bins = 200                          # training input quantization bins (NN)                   XXXXX
...
for scen_idx, n_beams, norm_type, noise, rep in combinations:
    np.random.seed(45)
    print('Experiment: {}'.format((scen_idx, n_beams, norm_type, noise, rep)))
    
    # MOHSEN ==========================================================================
    mohsen_run_indx = mohsen_run_indx + 1
    # =================================================================================
    ...
    # MOHSEN ========================================================================== 
    # We can add a randomization step for the beam indices 
    # beam_idxs = np.arange(0, max_beams)
    # np.random.shuffle(beam_idxs)
    # beam_idxs = beam_idxs[:n_beams]
    # =================================================================================
    ...
    for ai_strategy in ai_strategies:
        ...
        if run_idx > n_avgs:
            ...
            # MOHSEN: Train, validation, test split
            ...
            # MOHSEN ==========================================================================
            # Converting Data to be suitable for LSTMs input (making timeserieses)
            if ai_strategy == 'LSTM':
                ...
                for i in np.arange(n_past, len(pos_norm)):
                    X_lstm.append(pos_norm_sorted[(i-n_past):(i), :])
                    Y_lstm.append(beam_data_sorted[i-1])
            ...
            # MOHSEN ==========================================================================
            print(55*'*')
            print('Run Index: {}'.format(mohsen_run_indx))
            print(55*'*')
            # =================================================================================
        ...
        if ai_strategy == 'NN' and run_idx <= n_avgs:
            # MOHSEN: WHAT IS QUANTIZED INPUT?
            if quantize_input:
                n_bins = 200
